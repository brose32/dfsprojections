...
for year in range(2013, 2025):
    for race in range(1, 34):
        if race < 10:
            my_url = f"https://www.driveraverages.com/nascar_xfinityseries/race.php?sked_id={year}50{race}"
        else:
            my_url = f"https://www.driveraverages.com/nascar_xfinityseries/race.php?sked_id={year}5{race}"
        print(my_url)
        ...
        date = re.search('^.*(?=(\sXfinity Race Results))', date_text).group(0)
        ...
        for i in range(1, len(rows)):
            ...
            if row_data[13].text == '':
                driver_data.append({
                    ...
                    "lapsLedPercentage": float(row_data[7].text) / max(float(row_data[6].text), 1),
        ...
        db = client.nascar
        races_collection = db.races_xfinity
