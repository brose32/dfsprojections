...
game_times = page_soup.findAll("div", {"class": "cmg_game_time"})
tip_list = []
for time in game_times:
    idx = time.text.lstrip().find('M')
    tip_list.append(time.text.lstrip()[:idx + 1])
...
for team in home_teams:
    #in case there is a rescheudled game with no ID
...
lines_sheet.append(("HOME TEAM LINE", "O/U", "HOME TEAM", "HOME TOTAL", "AWAY TEAM", "AWAY TOTAL", "TIP"))
...
for i in range(0, len(lines)):
    ...
    lines_sheet.append((lines[i]['homespread'], lines[i]['total'], home_teams_list[i], home_total, away_teams_list[i],
                        (lines[i]['total'] - home_total), tip_list[i]))
wb.save(my_file)