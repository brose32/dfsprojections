...
class DKNBAOpt(DKNBAsetup):
    def __init__(self):
        ...
        self.MIN_SALARY = 49500
        self.overlap = 6
        self.overlap_lateswap = 6
        self.max_per_team = 3
    ...
    def lateSwap(self, lineups, locked, named_lineups):
        ...
        # prob += ((pulp.lpSum(self.player_names["Domantas Sabonis"][i] * player_lineup[i] +
        #         #                             self.player_names["De'Aaron Fox"][i] * player_lineup[i]
        ...
        # prob += ((pulp.lpSum((self.player_names['Luka Doncic'][k]*lineups[i][k])
        #                    for i in range(len(lineups)) for k in range(self.num_players)))
        #                    + (pulp.lpSum((self.player_names['Luka Doncic'][k]*
        #                        player_lineup[k] for k in range(self.num_players)))) <= (.7 * self.total_lineups))
